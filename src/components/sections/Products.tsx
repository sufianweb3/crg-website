      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16" role="list">
          {categories.map((category, index) => (
            <StaggerReveal key={category.id} delay={100 + index * 100} duration={400} tag="article" className="group relative hairline-border overflow-hidden" role="listitem">
              <Link
                href={`/products-library?category=${category.id}`}
                className="block p-6 lg:p-8 hover:bg-peach-black/3 transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-peach-black focus-visible:ring-offset-2 focus-visible:ring-offset-azure-mist"
              >
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-2 font-mono text-sm font-medium text-peach-black-45 group-hover:text-peach-black transition-colors duration-200 underline-draw">
                    View Collection
                    <svg className="w-4 h-4 flex-shrink-0 transition-transform duration-300 group-hover:translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                      <path d="M5 12h14M12 5l7 7-7 7" />
                    </svg>
                  </div>
                </div>

                <div className="absolute bottom-0 right-0 w-24 h-24 border-l border-b border-hairline opacity-0 group-hover:opacity-100 transition-opacity duration-300" aria-hidden="true" />
              </Link>
            </StaggerReveal>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <StaggerReveal delay={500} duration={400} tag="div">
            <Link
