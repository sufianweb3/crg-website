  ...
  return (
    <section className="section-padding bg-azure-mist" aria-labelledby="sustainability-heading">
      <div className="section-container">
        ...
        <StaggerReveal delay={100} duration={500} tag="div" className="max-w-3xl mt-8 body-text">
          <p>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" role="list">
          {certifications.map((cert, index) => (
            <StaggerReveal key={cert.id} delay={200 + index * 100} duration={400} tag="article" className="group hairline-border relative overflow-hidden" role="listitem">
              <button
                type="button"
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className="font-mono text-sm font-medium text-peach-black-45 group-hover:text-peach-black transition-colors duration-200">
                    View Details
                  </span>
                  <svg
                    className={`w-5 h-5 flex-shrink-0 ml-4 text-peach-black-45 transition-transform duration-300 ease-in-out ${expandedId === cert.id ? "rotate-180" : ""}`}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
              ...
              <div
                ...
                style={{ maxHeight: expandedId === cert.id ? "200px" : "0" }}
              >
                <p className="px-6 pb-6 body-text text-peach-black-70" style={{ opacity: expandedId === cert.id ? 1 : 0 }}>
                  {cert.description}
                </p>
              </div>
            </StaggerReveal>
        ...
        <div className="mt-16 p-8 border border-hairline">
          <StaggerReveal delay={600} duration={400} tag="div" className="max-w-3xl">
