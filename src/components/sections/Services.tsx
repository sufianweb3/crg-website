      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16" role="list">
          {services.map((service, index) => (
            <StaggerReveal key={service.id} delay={index * 100} duration={400} tag="article" className="hairline-border rounded-none relative group" role="listitem">
              <div className="p-6 lg:p-8 relative">
                <div className="absolute top-4 right-4 w-12 h-12 border border-hairline opacity-0 group-hover:opacity-100 transition-opacity duration-300" aria-hidden="true">
                ...
                <button
                  type="button"
                  className="w-full flex items-center justify-between text-left font-mono text-sm font-medium text-peach-black-45 hover:text-peach-black transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-peach-black focus-visible:ring-offset-2 focus-visible:ring-offset-azure-mist"
                ...
                >
                  ...
                  <svg
                    className={`w-5 h-5 flex-shrink-0 ml-4 transition-transform duration-300 ease-in-out ${expandedId === service.id ? "rotate-180" : ""}`}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
              ...
              <div
                ...
                className="overflow-hidden transition-all duration-300 ease-in-out bg-peach-black/5"
                style={{ maxHeight: expandedId === service.id ? "500px" : "0" }}
              >
                <ul className="px-6 pb-6 space-y-3" style={{ opacity: expandedId === service.id ? 1 : 0 }}>
